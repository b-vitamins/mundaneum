...
async def sync_imported_entries(entries: list[Entry]) -> None:
...
def ensure_search_index_ready() -> None:
...
async def ingest_parsed_entries(
    ...
    *,
    ...
    if sync_search:
        await sync_imported_entries(result.imported_entries)
...
async def ingest_directory(session: AsyncSession, directory: str | Path) -> dict:
    ...
    for bib_path in bib_files:
        file_result = await ingest_bib_file(session, bib_path)
