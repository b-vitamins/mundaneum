...
const healthResource = usePollingResource<AdminHealth>(() => api.getAdminHealth())
const health = healthResource.data
const loading = healthResource.loading
const error = computed(() => healthResource.error.value ? 'Could not load health status' : '')
...
const importMutation = useMutation(
  ...
  },
  {
    errorMessage: (loadError) =>
      loadError instanceof SyntaxError ? 'Invalid JSON file' : 'Failed to import backup',
...
async function exportBackup() {
  try { await exportMutation.execute() } catch (e) { console.error(e) }
}

async function triggerIngest() {
  ingestResult.value = null
  try { await ingestMutation.execute() } catch (e) { console.error(e) }
}
</script>
  ...
  <AppShell title="Admin">
    <div class="admin-content">
      ...
      <section class="admin-card card">
        ...
        <button class="btn btn-ghost" @click="healthResource.fetch" :disabled="loading">
      ...
      <section class="admin-card card">
        ...
        <button class="btn btn-primary" @click="triggerIngest" :disabled="ingestPending">
      ...
      <section class="admin-card card">
        ...
        <button class="btn btn-primary" @click="exportBackup" :disabled="exportPending">
          {{ exportPending ? 'Exporting...' : '💾 Export Backup' }}
        </button>
        <div v-if="exportSuccess" class="result result-success">✓ {{ exportSuccess }}</div>
        <div v-if="exportError" class="result result-error">{{ exportError }}</div>
      </section>
      ...
      <section class="admin-card card">
        ...
        <label class="btn btn-ghost file-label">
          <input type="file" accept=".json" @change="handleImport" :disabled="importPending" class="file-input" />
